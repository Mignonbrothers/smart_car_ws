...
YOLO 가중치는 .gitignore의 *.pt 규칙으로 저장소에 없습니다. pc/에 넣고 다시 빌드하면 setup.py의 data_files가 share/smart_car_py_pkg/models/로 설치하고 각 노드의 경로 탐색 로직이 찾습니다.
...
person_follower는 파라미터 콜백(onParameterUpdate)을 구현해 재빌드 없이 실시간 변경이 가능합니다.
...
pan_tilt_ros2의 제스처 임계값: pose_stationary_motion_threshold(0.03), pose_resume_knee_hip_tolerance(0.08), pose_stationary_min_keypoints(6)
